...
#' Combine SNAPE info from multiple (optionally gzipped) gSYNC files into one.
...
for type in TIME SPACE ALL; do
    ...
    OUT_BASE=snape_masked_$(echo "${type}" | tr '[:upper:]' '[:lower:]')
    OUT=${OUT_BASE}.sync.gz
    OUT_NAMES=${OUT_BASE}.names.gz
    OUT_NOBLANKS=${OUT_BASE}_noblanks.sync.gz

    # Combine snape files (takes ~17 min)
    ./combine-snapes.py -o ${OUT%.gz} ${SAMPS[@]/%/.sync.gz}
    check_exit_status "combine-snapes.py ($type)" $?
    ...
    # Create snape file with no blanks
